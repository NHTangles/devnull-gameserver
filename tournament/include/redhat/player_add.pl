...
sub player_add
{
	...
	if(0 != $uid)
	{
		$cmd_add = "/usr/sbin/useradd -c \"DevNull NetHack Player\" -d $tournament_home/players/$player -g nhplayer -m -p $passwd -s $tournament_home/bin/nethack.login -u $uid $player";
